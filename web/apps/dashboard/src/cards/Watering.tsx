...
const outcomeLabel: Record<WateringEvent["outcome"], string> = { completed: "completed", aborted: "aborted", dry_run: "dry run", recovery: "recovery" };
...
export function Watering() {
  ...
  return <Card className="card-watering"><CardTitle icon={icon}>Watering history</CardTitle>
    <ul className="list-none m-0 p-0 text-[0.9rem] max-h-72 overflow-y-auto empty:after:content-['No_waterings_recorded_yet.'] empty:after:block empty:after:py-2 empty:after:italic">
      {events.map((event) => <li key={event.id} className={`flex items-baseline gap-4 py-[0.5rem] border-b-[2px] border-dashed last:border-b-0 ${event.outcome === "completed" ? "border-gray" : "border-danger"}`}>
        <time className="font-num tabular-nums font-extrabold">{event.startedAt ? new Date(event.startedAt).toLocaleString() : "time unknown"}</time>
        <span className="font-bold uppercase text-[0.72rem] tracking-[0.06em]">{duration(event.startedAt, event.endedAt)}</span>
        <span className={`uppercase text-[0.72rem] font-extrabold tracking-[0.06em] ${event.outcome === "completed" ? "" : "text-danger"}`}>{outcomeLabel[event.outcome]}{event.trigger === "manual" ? " · manual" : ""}</span>
        <span className="font-num font-extrabold text-[1.1rem] ml-auto">{event.litresDelivered.toFixed(1)}<i className="not-italic text-[0.72rem] font-extrabold ml-[0.35rem]">L</i></span>
      </li>)}
    </ul></Card>;
}
